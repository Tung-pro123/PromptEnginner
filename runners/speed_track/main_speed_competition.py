...
class CompetitionSpeedController:
    def __init__(self):
        ...
        # Khung hình không nén để giảm trễ giải mã nếu có thể
    ...
    def detect_lane(self, frame):
        ...
        if abs(self.last_steer) > 0.5:
            w_f, w_m, w_n = 0.40, 0.40, 0.20 # Cua gắt thì nhìn gần lại 1 chút để giữ xe không lao ra ngoài
            
        target_x = mid_n * w_n + mid_m * w_m + mid_f * w_f
        
        # Apex Bias: Khuếch đại mục tiêu về phía khúc cua để chủ động bám góc trong (cắt cua)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.state == TrackState.WAITING:
                ...
                if self.time_in_state() > 2.0: # Cooldown time before accepting new checkpoint
                    self.set_state(TrackState.RACING)
