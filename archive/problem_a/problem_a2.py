...
class JetBotController:
    def __init__(self):
        ...
        # Pass turn_cost to map navigator to prefer straighter paths
        try:
            self.navigator = MapNavigator((self.MAP_FILE_PATH, self.TURN_COST))
        except Exception:
            # fallback
        ...
        # smoothing state for line-following
        self.last_adj = 0.0
        ...
        # Ensure safe stop on ROS shutdown
        try:
            rospy.on_shutdown(self._on_shutdown)
        except Exception:
            # rospy may not be fully initialized in some test environments
            pass
    ...
    def setup_parameters(self):
        ...
        # Default starting heading: East
        # Use index lookup so this remains correct even if DIRECTIONS order changes
        try:
            self.current_direction_index = self.DIRECTIONS.index(Direction.EAST)
        except ValueError:
            # Fallback to index 1 if Direction.EAST isn't found for some reason
        ...
        # After stopping in intersection, how long to wait to re-confirm sensors
        self.INTERSECTION_CONFIRM_TIMEOUT = 1.0
        ...
        self.prev_error = 0.0
        self.prev_time = None
        # LiDAR confirmation and node-counting to avoid false positives at start
        self.LIDAR_CONFIRM_REQUIRED = 2
        self.lidar_confirm_count = 0
        # Minimum number of nodes (excluding the start node) to have passed before honoring LiDAR
        self.MIN_NODES_BEFORE_LIDAR = 2
        # Track how many nodes we've moved from the starting node (do not count the starting node)
        self.nodes_since_start = 0
        # Motor error flag (set if hardware I/O fails) — used to abort safely
        self.motor_error = False
        # Stabilize / reacquire tuning
        self.STABILIZE_SMALL_ANGLE = 10
        self.STABILIZE_ATTEMPTS = 4
        self.STABILIZE_SLEEP = 0.08
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            # If a motor I/O error occurred, stop the loop safely to avoid runaway
            if getattr(self, 'motor_error', False):
                rospy.logerr("Motor I/O error detected — aborting main loop for safety.")
                break
            # ===================================================================
            ...
            if self.current_state == RobotState.DRIVING_STRAIGHT:
                ...
                if time_since_state >= self.MIN_DETECTION_CONFIRM_TIME:
                    if self.detector.process_detection():
                        self.lidar_confirm_count += 1
                    else:
                        self.lidar_confirm_count = 0

                if self.lidar_confirm_count >= self.LIDAR_CONFIRM_REQUIRED and self.nodes_since_start >= self.MIN_NODES_BEFORE_LIDAR:
                    # Determine planned action at current node (if available)
                    planned_label = None
                    try:
                    ...
                    planned_action = None
                    if planned_label:
                        planned_action = self.map_absolute_to_relative(planned_label, self.DIRECTIONS[self.current_direction_index])

                    if planned_action == 'straight':
                        rospy.loginfo("SỰ KIỆN (LiDAR): Giao lộ phía trước nhưng kế hoạch đi thẳng -> không dừng.")
                        # reset confirmation so we don't trigger again immediately
                        self.lidar_confirm_count = 0
                    else:
                        rospy.loginfo("SỰ KIỆN (LiDAR): Giao lộ xác nhận sau %d lần. Chuẩn bị tiến vào giao lộ.", self.lidar_confirm_count)
                        # chuyển sang APPROACHING để tiến vào trung tâm giao lộ trước khi xử lý
                        self._set_state(RobotState.APPROACHING_INTERSECTION)
                        # reset confirmation so next intersection needs re-confirm
                        self.lidar_confirm_count = 0
                        continue # bắt đầu vòng lặp mới để thực hiện APPROACHING_INTERSECTION
            ...
            # ===================================================================
                ...
                if rospy.get_time() - self.state_change_time > self.INTERSECTION_APPROACH_DURATION:
                    ...
                    # stop and attempt to confirm intersection with LiDAR + camera
                    try:
                        self.safe_stop()
                    except Exception:
                        pass
                    ...
                    # Update current node and increment the nodes-since-start counter
                    self.current_node_id = self.target_node_id
                    self.nodes_since_start = getattr(self, 'nodes_since_start', 0) + 1
                    rospy.loginfo(f"==> ĐÃ ĐẾN node {self.current_node_id}. (nodes_since_start={self.nodes_since_start})")

                    # Try to confirm intersection presence for a short timeout window
                    confirmed = False
                    start_confirm = rospy.get_time()
                    while rospy.get_time() - start_confirm < self.INTERSECTION_CONFIRM_TIMEOUT:
                        if self.detector.process_detection() and self._does_path_exist_in_frame(self.latest_image):
                            confirmed = True
                            break
                        rospy.sleep(0.05)

                    # If LiDAR+camera didn't confirm, try a quick proactive camera scan (small-angle sweeps)
                    if not confirmed:
                        rospy.loginfo("LiDAR+camera không xác nhận. Thử quét chủ động bằng camera...")
                        try:
                            scan_paths = self.scan_for_available_paths_proactive()
                        except Exception as e:
                            rospy.logwarn(f"Quét chủ động lỗi: {e}")
                            scan_paths = {"straight": False, "left": False, "right": False}

                        rospy.loginfo(f"Kết quả quét chủ động: {scan_paths}")

                        # Determine planned action to check if scan confirms planned branch
                        planned_label = None
                        try:
                            planned_label = self.navigator.get_next_direction_label(self.current_node_id, self.planned_path)
                        except Exception:
                        ...
                        if planned_action and planned_action in scan_paths and scan_paths.get(planned_action, False):
                            rospy.loginfo(f"Quét chủ động xác nhận đường cho hành động đã lập: {planned_action}")
                            confirmed = True

                    if not confirmed:
                        rospy.logwarn("Không xác nhận giao lộ sau khi vào giữa. Tiếp tục rời giao lộ (không xử lý).")
                        # resume leaving intersection and try to reacquire line
            ...
            # ===================================================================
                ...
                try:
                    self.safe_set_motors(self.BASE_SPEED, self.BASE_SPEED)
                except Exception:
            ...
            # ===================================================================
                ...
                # Try a few stabilization / scan passes before giving up
                ...
                # Attempt stabilizing small sweeps and proactive scans
                reacquired = False
                for attempt in range(getattr(self, 'STABILIZE_ATTEMPTS', 4)):
                    if self.stabilize_after_turn(max_attempts=1, small_angle=getattr(self, 'STABILIZE_SMALL_ANGLE', 10)):
                        line_center_x = self._get_line_center(self.latest_image, self.ROI_Y, self.ROI_H)
                        if line_center_x is not None:
                            rospy.loginfo("Reacquired line after stabilize sweep.")
                            reacquired = True
                            break
                    # proactive camera scan
                    scan_paths = self.scan_for_available_paths_proactive()
                    if scan_paths.get('straight'):
                        # if straight path exists, try to follow it
                        rospy.loginfo("Proactive scan found straight path during reacquire.")
                        reacquired = True
                        break
                    rospy.sleep(getattr(self, 'STABILIZE_SLEEP', 0.08))
    ...
    def cleanup(self):
        ...
        if hasattr(self, 'robot') and self.robot is not None:
            try:
                ...
                # If safe_stop fails, try raw stop as a last resort
                try:
                    self.robot.stop()
                except Exception:
                    pass
    ...
    def correct_course(self, line_center_x):
        ...
        p = self.P_GAIN * (error / (self.WIDTH / 2))
        d = 0.0
        d = self.D_GAIN * ((error - self.prev_error) / dt) if dt > 0 else 0.0

        raw_adj = p + d
        raw_adj = float(np.clip(raw_adj, -self.MAX_CORRECTION_ADJ, self.MAX_CORRECTION_ADJ))

        # smoothing on adj (low-pass)
        alpha = getattr(self, 'SMOOTHING_ALPHA', 0.45)
        new_adj = alpha * raw_adj + (1 - alpha) * getattr(self, 'last_adj', 0.0)
        self.last_adj = new_adj

        left_motor = self.BASE_SPEED + new_adj
        right_motor = self.BASE_SPEED - new_adj
        left_motor = float(np.clip(left_motor, self.MIN_MOTOR, self.MAX_MOTOR))
        right_motor = float(np.clip(right_motor, self.MIN_MOTOR, self.MAX_MOTOR))

        try:
            self.safe_set_motors(left_motor, right_motor)
        except Exception as e:
            rospy.logerr(f"Lỗi motor khi set_motors: {e}")
            self.motor_error = True
    ...
    def handle_intersection(self):
        ...
        # Ensure `current_node_id` is set. Normally it's updated in APPROACHING_INTERSECTION
        # when we drive into the center. But if we reach here (e.g., approach was skipped),
        # assign it now to avoid inconsistent state. Only increment nodes_since_start when
        # we actually change the node here to avoid double-counting.
        if getattr(self, 'target_node_id', None) is not None and self.current_node_id != self.target_node_id:
            self.current_node_id = self.target_node_id
            self.nodes_since_start = getattr(self, 'nodes_since_start', 0) + 1
            rospy.loginfo(f"==> ĐÃ ĐẾN node {self.current_node_id}. (nodes_since_start={self.nodes_since_start})")
        else:
            rospy.loginfo(f"==> Xử lý tại node {self.current_node_id}.")
    ...
    # --- Motor safety wrappers -------------------------------------------------
    def safe_set_motors(self, left, right):
        """Wrapper that calls robot.set_motors and translates OSError into exception flag."""
        if getattr(self, 'robot', None) is None:
            return
        try:
            self.robot.set_motors(left, right)
        except OSError as e:
            # propagate to caller to handle motor_error
            raise
    ...
    def turn_robot(self, degrees, update_main_direction=True):
        ...
        if degrees > 0: 
            try:
                self.safe_set_motors(self.TURN_SPEED, -self.TURN_SPEED)
            except Exception as e:
                rospy.logerr(f"Lỗi motor khi quay: {e}")
                self.motor_error = True
                return
        elif degrees < 0: 
            try:
